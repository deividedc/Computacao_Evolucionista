...
pkl_list   += glob.glob('./pkl_yudongnan/loo*.pkl')
...
#for (d,f), df in tbl.groupby(['Dataset','Phase', ]):
    ...
    print(df1)
    df1.to_latex(buf=fn, index=False, escape=False, label=fn, caption='', column_format='r'*df1.shape[1])
    df_table.append(df1)
...
for(d,df) in C.groupby('Dataset'):
    ...
    for kind in ['bar', 'box',]:# 'violin']:
        for m in metrics:
            kwargs={'edgecolor':"k", 'capsize':0.05, 'alpha':0.95, 'ci':'sd', 'errwidth':1.0, 'dodge':True, 'aspect':1, 'legend':None, } if kind=='bar' else {'notch':1, 'ci':'sd','aspect':1.0618,}
    #        sns.catplot(x='Dataset', y='R$^2$', data=C, hue='Phase', kind='bar', col='Estimator')
    ...
    #                       **kwargs,);
            if kind=='bar':
                g=sns.catplot(x='Output', y=m, hue='Estimator', #row='Phase', 
...
for i, j in zip(estimators,['r', 'darkgreen', 'b', 'm', 'c','y', 'olive',  'darkorange', 'brown', 'darkslategray', ]): 
    colors[i]=j
